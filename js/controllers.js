angular.module('starter.controllers', [])
    .controller('menuCtrl', function ($scope, $rootScope, $ionicScrollDelegate, $ionicLoading, $http, $filter, $ionicHistory, $ionicViewService, $cordovaSQLite, $document, $cordovaNetwork, $ionicPopup, $state, $cordovaToast) {
...
.controller('orderlistCtrl', function ($scope, $rootScope, $filter, $ionicLoading, $cordovaSQLite, $ionicPopup, $cordovaKeyboard) {
    ...
    $scope.filterbyMonth = function (month) {
        ...
        var pastmonth = cmonth - month;
        var pDate = cDate.setMonth(pastmonth);

        $cordovaSQLite.execute($rootScope.DB, 'SELECT * FROM order_history WHERE accountNumber = "' + $rootScope.mAccount.AccountNumber + '" AND orderHistoryNumericDate>="' + pDate + '"')
            .then(
                function (result) {
                    $rootScope.orderhList = [];
                    if (result.rows.length > 0) {
